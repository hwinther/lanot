...
cp -v ports/esp8266/build-NODEMCUESP12E/firmware-combined.bin ../../deploy/micropython/esp8266/NODEMCUESP12E-$mpyversion-p$prometheus_version-$mpybuilddate.bin
esp8266-make -C ports/esp8266 -j BOARD=LOLINESP32E
cp -v ports/esp8266/build-LOLINESP32E/firmware-combined.bin ../../deploy/micropython/esp8266/LOLINESP32E-$mpyversion-p$prometheus_version-$mpybuilddate.bin
...
printf "${BLUE}*******************************************${NC}\n"
esp32-make() {
    sudo docker run --rm -v $(pwd)/../../:/opt/lanot -w /opt/lanot -e IDF_PATH=/opt/sdk/esp-idf --entrypoint bash --privileged -u 0 ghcr.io/hwinther/lanot/esp-idf-5:5.0.2 -c "set -x && source /opt/sdk/esp-idf/export.sh && git config --global --add safe.directory '*' && mkdir -p /opt/local && cp -rd /opt/lanot /opt/local/lanot && cd /opt/local/lanot/tools/micropython && git describe --tags --dirty --always && git status --porcelain && cd ports/esp32 && make BOARD=NODEMCU32S submodules && make BOARD=NODEMCU32S && echo \"esp32 NODEMCU32S version defines:\" && cat build-NODEMCU32S/genhdr/mpversion.h && find . -type f \( -name \*.bin -or -name \*.elf -or -name \*.map -or -name sdkconfig -or -name flash_args \) -path \*/build-\*/\* -not -path \*/CMakeFiles/\* -exec cp --parents {} /opt/lanot/tools/micropython/ports/esp32/ \;"
}
esp32-make
