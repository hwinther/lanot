...
printf "${BLUE}*****************************${NC}\n"
mpy-make() {
    sudo docker run --rm -it -v $(pwd)/../../:/opt/lanot -w /opt/lanot/tools/micropython --entrypoint make ghcr.io/hwinther/lanot/mpy-cross:13 "$@"
}
...
printf "${BLUE}*******************************************${NC}\n"
esp8266-make() {
    sudo docker run --rm -it -v $(pwd)/../../../../:/opt/lanot -w /opt/lanot/tools/micropython/ports/esp8266 --entrypoint make ghcr.io/hwinther/lanot/esp-open-sdk:2.1.0 "$@"
}
...
printf "${BLUE}*******************************************${NC}\n"
cc3200-make() {
    sudo docker run --rm -it -v $(pwd)/../../../../:/opt/lanot -w /opt/lanot/tools/micropython/ports/cc3200 --entrypoint make ghcr.io/hwinther/lanot/gcc-arm-none-eabi:10.3-2021.10 "$@"
}
