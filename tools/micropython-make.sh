...
printf "${BLUE}*******************************************${NC}\n"
make axtls && make && cp build/firmware-combined.bin ../../../../deploy/micropython/esp8266/$mpyversion-$mpybuilddate.bin

printf "${BLUE}*******************************************${NC}\n"
printf "${BLUE}* Making cc3200-WIPY $mpyversion-$mpybuilddate *${NC}\n"
printf "${BLUE}*******************************************${NC}\n"
cd ../cc3200
make BTARGET=bootloader BTYPE=release BOARD=WIPY && cp bootmgr/build/WIPY/release/bootloader.bin ../../../../deploy/micropython/wipy/custom/bootloader-$mpyversion-$mpybuilddate.bin
make BTARGET=application BTYPE=release BOARD=WIPY && cp build/WIPY/release/mcuimg.bin ../../../../deploy/micropython/wipy/custom/mcuimg-$mpyversion-$mpybuilddate.bin && unlink ../../../../deploy/micropython/wipy/custom/mcuimg.bin && ln -s ../../../../deploy/micropython/wipy/custom/mcuimg-$mpyversion-$mpybuilddate.bin ../../../../deploy/micropython/wipy/custom/mcuimg.bin
