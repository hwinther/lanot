...
class Sensor02UdpClientRedLed(prometheus.Prometheus):
    def __init__(self, send, recv):
        prometheus.Prometheus.__init__(self)
        self.send = send
        self.recv = recv

    @prometheus.Registry.register('Sensor02UdpClientRedLed', 'r0')
    def off(self):
        self.send(b'r0')

    @prometheus.Registry.register('Sensor02UdpClientRedLed', 'r1')
    def on(self):
        self.send(b'r1')

    @prometheus.Registry.register('Sensor02UdpClientRedLed', 'rS', 'OUT')
    def state(self):
        self.send(b'rS')
        return self.recv(10)
...
class Sensor02UdpClientIntegratedLed(prometheus.Prometheus):
    ...
    @prometheus.Registry.register('Sensor02UdpClientIntegratedLed', 'iS', 'OUT')
    def state(self):
        self.send(b'iS')
        return self.recv(10)


class Sensor02UdpClient(prometheus.RemoteTemplate):
    def __init__(self, remote_host, remote_port=9195, bind_host='', bind_port=9195):
        ...
        self.red_led = Sensor02UdpClientRedLed(self.send, self.recv)
...
class Sensor02RsaUdpClientDht11(prometheus.Prometheus):
    def __init__(self, send, recv):
        prometheus.Prometheus.__init__(self)
        self.send = send
        self.recv = recv

    @prometheus.Registry.register('Sensor02RsaUdpClientDht11', 'dh', 'OUT')
    def humidity(self):
        self.send(b'dh')
        return self.recv(10)

    @prometheus.Registry.register('Sensor02RsaUdpClientDht11', 'dm')
    def measure(self):
        self.send(b'dm')

    @prometheus.Registry.register('Sensor02RsaUdpClientDht11', 'dt', 'OUT')
    def temperature(self):
        self.send(b'dt')
        return self.recv(10)

    @prometheus.Registry.register('Sensor02RsaUdpClientDht11', 'dv', 'OUT')
    def value(self):
        self.send(b'dv')
        return self.recv(10)


class Sensor02RsaUdpClientLightsensor(prometheus.Prometheus):
    def __init__(self, send, recv):
        prometheus.Prometheus.__init__(self)
        self.send = send
        self.recv = recv

    @prometheus.Registry.register('Sensor02RsaUdpClientLightsensor', 'lr', 'OUT')
    def read(self):
        self.send(b'lr')
        return self.recv(10)


class Sensor02RsaUdpClientRedLed(prometheus.Prometheus):
    def __init__(self, send, recv):
        prometheus.Prometheus.__init__(self)
        self.send = send
        self.recv = recv

    @prometheus.Registry.register('Sensor02RsaUdpClientRedLed', 'r0')
    def off(self):
        self.send(b'r0')

    @prometheus.Registry.register('Sensor02RsaUdpClientRedLed', 'r1')
    def on(self):
        self.send(b'r1')

    @prometheus.Registry.register('Sensor02RsaUdpClientRedLed', 'rS', 'OUT')
    def state(self):
        self.send(b'rS')
        return self.recv(10)


class Sensor02RsaUdpClientIntegratedLed(prometheus.Prometheus):
    def __init__(self, send, recv):
        prometheus.Prometheus.__init__(self)
        self.send = send
        self.recv = recv

    @prometheus.Registry.register('Sensor02RsaUdpClientIntegratedLed', 'i0')
    def off(self):
        self.send(b'i0')

    @prometheus.Registry.register('Sensor02RsaUdpClientIntegratedLed', 'i1')
    def on(self):
        self.send(b'i1')

    @prometheus.Registry.register('Sensor02RsaUdpClientIntegratedLed', 'iS', 'OUT')
    def state(self):
        self.send(b'iS')
        return self.recv(10)


class Sensor02RsaUdpClient(prometheus.RemoteTemplate):
    def __init__(self, remote_host, remote_port=9195, bind_host='', bind_port=9195, clientencrypt=False):
        ...
        self.dht11 = Sensor02RsaUdpClientDht11(self.send, self.recv)
        self.register(dht11=self.dht11)
        self.integrated_led = Sensor02RsaUdpClientIntegratedLed(self.send, self.recv)
        self.register(integrated_led=self.integrated_led)
        self.lightsensor = Sensor02RsaUdpClientLightsensor(self.send, self.recv)
        self.register(lightsensor=self.lightsensor)
        self.red_led = Sensor02RsaUdpClientRedLed(self.send, self.recv)
        self.register(red_led=self.red_led)
